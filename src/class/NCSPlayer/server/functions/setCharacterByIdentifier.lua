...
function NCSPlayer.setCharacterByIdentifier(self, characterId, callback)
    API_Database:query("SELECT * FROM ncs_players_characters WHERE character_id = @character_id", {
        ...
        ---@type NCSCharacter
        local newCharacter = NCSCharacter(characterId, json.decode(row.identity), json.decode(row.skin), json.decode(row.accounts), json.decode(row.last_position), json.decode(row.metadata), row.is_dead, row.death_cause)
                :setUser(self)
